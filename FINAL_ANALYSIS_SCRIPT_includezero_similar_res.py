...
version='includeWtandNull_grouped_res'
# 20 amino acids ...
...
cors_dict['identical_aa'] = pd.Series(all_corrs)
 ...
 ##########################################
# # corrs averaging by the similar hydrophobicity AA ...
# ###########################################
KD = ProtParamData.kd
KDvalues = set()
...
all_corrs = []
for kd in KDvalues:
   yyy = dat[aacids][dat['wtKD']==kd]
   cmat = yyy.T.corr('pearson')
   n,n = cmat.shape
   tril_values = cmat.as_matrix()[np.tril_indices(n,-1)]
   avg_corr = tril_values.mean()
   avg_corrs.append((kd,avg_corr,n,0.5*n*(n-1)))
   # print aa,avg_corr
   all_corrs.append(tril_values)
   
all_corrs = np.hstack(all_corrs)
avg_corrs_df = pd.DataFrame(avg_corrs,columns=['kd','r','num','num_triangle'])
avg_corrs_df.to_csv('./numbers_for_paper/avg_corrs_similar_AA_%s.csv' %(version),index=False)
print "Median corr(R) over all similar AAs: ", np.median(all_corrs)
print "Mean corr(R) over all similar AAs: ", all_corrs.mean()
print "Mean corr(R) over all similar AAs(not weighted): ", avg_corrs_df['r'].mean()

with open('./numbers_for_paper/avg_corrs_similar_AA_%s.csv' %(version),'a') as fp:
   fp.write('\nmedian weighted, %.4f\nmean weighted, %.4f\nmean non-weighted, %.4f'%(np.median(all_corrs), all_corrs.mean(),avg_corrs_df['r'].mean()))
...
# ##########################################
# # corrs averaging by structurally identical positions and similar hydrophobic wt ...
# ###########################################

avg_corrs = []
all_corrs = []
for pos in range(1,97):
    grp = dat[dat['align_orth_1-96']==pos].groupby('wtKD')
    for wd,idxs in grp.groups.iteritems():
       if len(idxs)>1:
           yyy = grp.get_group(wd)[aacids]
           cmat = yyy.T.corr('pearson')
           n,n = cmat.shape
           tril_values = cmat.as_matrix()[np.tril_indices(n,-1)]
           avg_corr = tril_values.mean()
...
avg_corrs_df = pd.DataFrame(avg_corrs,columns=['pos','r','num','num_triangle'])
avg_corrs_df.to_csv('./numbers_for_paper/avg_corrs_ident_pos_similar_aa_%s.csv' %(version),index=False)
# # # avg_corrs_df
print "Median corr(R) over all identical structural positions and similar AA: ", np.median(all_corrs)
print "Mean corr(R) over all identical structural positions and similar AA: ", all_corrs.mean()
print "Mean corr(R) over all identical structural positions and similar AA(not weighted): ", avg_corrs_df['r'].mean()

with open('./numbers_for_paper/avg_corrs_ident_pos_similar_aa_%s.csv' %(version),'a') as fp:
 fp.write('\nmedian weighted, %.4f\nmean weighted, %.4f\nmean non-weighted, %.4f'%(np.median(all_corrs), all_corrs.mean(),avg_corrs_df['r'].mean()))
